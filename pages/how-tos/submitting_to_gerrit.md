...
{% include alerts/note.html content="Creating a Gerrit account requires you to log in with GitHub or Google. Be aware that if you delete or unlink all GitHub and Google accounts associated with your Gerrit account, you won't be able to log in anymore!" %}
...
{%- capture content %}If you are looking for things to work on, check out the list of [open platform issues](https://github.com/LineageOS/issues/issues?q=is%3Aissue%20state%3Aopen%20label%3Aplatform). We do not have a feature request tracker, so if you would like to implement something, simply do so, and submit it. If it aligns with our project standards, we will gladly accept it.{% endcapture %}
