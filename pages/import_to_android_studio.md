...
After opening the project, navigate to `File->Project Structure->SDKs` (this might be disabled until the indexing has finished) and set up a JDK (version 1.8 for LineageOS 14.1 branch) where you remove _all_ libraries (`.jar` files). Press `Apply` and closse the settings.
