...
3. <div class="img-container">
    ...
    From here you can use the **Reboot recovery** or **Reboot bootloader** buttons to switch the device into the mode you need.

4. In LineageOS recovery, select `Apply update > Apply from ADB` using the touch screen, or use the volume keys to navigate and the power button to select.
5. Click **Connect** again if the device disconnected during the reboot, then select it from the pop-up list.
