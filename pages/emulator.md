...
In case that you don't have an officially supported device, you don't want to test changes on your daily driver, or you are just someone who wants to test apps with LineageOS-specific features, we've still got you covered.

These instructions will help you with building an emulator-compatible version of LineageOS, ready to run on your computer. If you want to use Android Studio/AVD, there are also instructions for packing up/installing your
...
For starting, `x86` is recommended (since your computer can run it using hardware acceleration).

Instead of `eng` one can also use `userdebug` (the latter one is used by official AOSP images), but ADB (or other communication between the emulator and the emulated device) will not work out-of-the-box.
...
After a few seconds, you will hopefully have a running version of LineageOS on your PC, ready to test things on.
