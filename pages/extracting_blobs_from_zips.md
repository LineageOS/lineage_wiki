...
You will now use a payload bin extractor. To use the tool, you will need python3-protobuf, if you do not already have it:

```
sudo apt-get install python3-protobuf
```

You can now extract the `.img` files from the payload.bin that's inside the lineage-*.zip:

```
python /path/to/lineage-tree/tools/extract-utils/extract_ota.py path/to/lineage-*.zip
```
