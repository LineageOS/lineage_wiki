...
You will now need a tool called `extract_android_ota_payload`. This script will extract the various `*.img` files contained within the `payload.bin` file. It can be obtained by cloning the following Github repository:

```
git clone https://github.com/cyxx/extract_android_ota_payload
```

To use the tool, you will also need python3 and python3-protobuf, if you do not already have it:

```
sudo apt-get install python3
sudo python3 -m pip install -r extract_android_ota_payload/requirements.txt
```

You can now extract the `.img` files:

```
python3 extract_android_ota_payload/extract_android_ota_payload.py payload.bin .
```

It will take a few moments. Once it's done, we will need to mount both the `system.img` file and the `vendor.img` file to obtain the complete set of proprietary blobs:
