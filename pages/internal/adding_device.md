...
* `install_method`: Used to determine the recovery install template to use. Templates can be found in \_includes/templates/recovery\_install\_`install_method`.md and must be one of:
...
* `before_install`: Sometimes some pre-conditions must be met in order to be able to install LineageOS. This often goes with `before_install_args`. See `_includes/templates/device_specific` for possible values.
