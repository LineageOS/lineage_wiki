...
* `bluetooth`: The proper format is either `{spec: '<version>'}` with `version` being the version of the BT protocol supported, or `{spec: '<version>', profiles: '<profiles>'}` when your device
  ...
  ```
* `burn_boot`: Instructions for booting the device into the mode used to flash low-level images to prepare the NAND/eMMC/UFS to boot Android. Most devices will not need to set this.
