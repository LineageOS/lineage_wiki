...
Additionally there are some optional properties which you might not need, but in case you do, here is their explanation:
...
* `custom_twrp_codename`: If an official TWRP exists for the device, but TWRP uses an other codename, specify the used one here. Remove this if not used!
