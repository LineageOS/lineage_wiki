...
Starting with LineageOS 22, you can now use the new Python extract utilities. They’re faster and more versatile. To start, decide which version of the extract utilities (if applicable) you want to use.

If you choose the shell scripts, copy the ```extract-files.sh``` and ```setup-makefiles.sh``` templates from ```tools/extract-utils/templates``` (```vendor/lineage/build/templates``` on LineageOS 17.1 and older) to your device tree and edit them to fill in the three required fields (device, vendor, and copyright year); alternatively, if you prefer the Python extract utilities, copy the ```extract-files.py``` and ```setup-makefiles.py``` templates from the same directory.
