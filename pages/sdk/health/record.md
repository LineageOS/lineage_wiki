...
| 106 | WeightRecord                 | Weight (can convert to mass in kg) | float (N), timestamp            |
{: .table }
...
| 303 | StepsRecord      | Number of steps     | integer (steps), timestamp (begin time), timestamp (end time)            |
| 304 | WorkoutRecord    | Workout exercise    | integer (calories), string, timestamp (begin time), timestamp (end time) |
{: .table }
...
val healthStore = HealthStore.getInstance(context)
val result = healthStore.writeRecord(myRecord)
if (result) {
    onSuccess()
} else {
    onError()
...
// save
final HealthStore healthStore = HealthStore.getInstance(context);
final boolean result = healthStore.writeRecord(myMedicalProfile);
if (result) {
    onSuccess();
} else {
    onFailure();
