...
Make sure you add the files that you've modified by using `git add`. Once you're satisfied, prepare the upload, by ammending your commit:
...
You can do `git log` and `git status` again. Notice how git handles your initial commit and the ammended commit as one single patch. As for `git show`, it shows you all the changes made on that commit.
