...
- When booted into Launcher, Open Settings app, Enable Developer options, Go back to homepage, Navigate to `System` > `Developer options`,
Open `DSU Loader`, Select the DSU Package you wish to install, Click `Agree`. Once the installation finishes, You can enter the GSI by clicking `Restart` on `Dynamic System Updates` notification.

- Download a GSI image archive (with matching architecture) from [Generic System Image releases](https://developer.android.com/topic/generic-system-image/releases), Extract `system.img` from the downloaded archive,
