...
1. On `Video` tab, select `Virtio` on `Model` menu, click `Apply`.
2. If the PC and the Remote Desktop application (if you're viewing from it) supports 3D Accelerated graphics, Toggle `3D acceleration`, click `Apply`.

#### Display

1. Open `Display` tab.
2. Select `None` on `Listen type` menu.
3. If `3D acceleration` is enabled on `Video` tab, toggle `OpenGL`, and select a active host video card on the menu below of `OpenGL` toggle.
4. Click `Apply`.
