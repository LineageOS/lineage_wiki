...
5. Click Finish.
6. On `Boot Options` tab, toggle `USB Disk 1`, click `Apply`.
...
4. If `3D acceleration` is enabled on `Video` tab, toggle `OpenGL`, and select a active host video card on the menu below of `OpenGL` toggle.
5. Click `Apply`.
