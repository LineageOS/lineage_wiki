...
Virtual A/B partition scheme is used by default. If you would like to use A-only partition scheme (which requires less space), execute the following command:
```
...
{% include alerts/important.html content="If you wish to run the virtual machine on ARMv9 PCs with hardware acceleration, you shall choose `virtio_arm64only` target. The target `virtio_arm64` would not boot." %}
...
5. Click Finish.
6. On `Boot Options` tab, toggle `SATA CDROM 1` or `USB Disk 1`, click `Apply`.
...
4. Once the installation finishes, you can reboot to the GSI by clicking `Restart` on `Dynamic System Updates` notification.
