...
```
fastboot -s tcp:<IPv4 address that shown on menu header> [fastboot commands...]
```
...
When in recovery mode, you'll be able to select custom flashable ZIPs from the shared directory through `Apply update` > `Choose from virtiofs`.

#### Insert a USB drive

Put the custom flashable ZIPs onto a USB drive, and add the USB drive into the virtual machine.

When in recovery mode, enter `Apply update`, volumes of the USB drive should appear. Enter the corresponding volume, and then select the custom flashable ZIP which you want to install.

### Integrate Magisk

{% include alerts/important.html content="Flashing Magisk in recovery mode will NOT work, as these targets do not use ramdisk from the `boot` partition. Please follow the below instructions if you want to use Magisk on these targets." %}
...
* The first serial console is used for showing GRUB text menu and printing kernel messages.
