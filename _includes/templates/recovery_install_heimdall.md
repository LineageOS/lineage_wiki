...
4. A blue transfer bar will appear on the device showing the recovery image being flashed.
    {% include alerts/tip.html content="Note that the phone will continue to display \"Downloading... Do not turn off target!!\" even after the download is finished. When the status message in the top left on the phone says reports the process is complete, you can continue." %}
