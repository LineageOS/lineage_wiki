...
The preferred method of installing a custom recovery is through this boot mode{% if site.data.devices[page.device].root_method[0] != 'custom' %} -- rooting the stock firmware is neither necessary nor required{% endif %}.
...
{% if site.data.devices[page.device].root_method[0] == 'custom' %}
