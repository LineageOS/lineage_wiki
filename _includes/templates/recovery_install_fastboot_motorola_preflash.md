...
```
    ...
    You can also boot into bootloader mode via a key combination:
...
## Installing required partitions using `fastbootD`

1. Download [Lineage Recovery and Boot](https://download.lineageos.org/devices/{{ device.codename }}).
...
```
    {% if device.download_boot %}
    You can also boot into fastbootD mode via bootloader mode:
...
```
fastboot flash vendor_boot {{ device.recovery_partition_name }}.img
```
7. Reboot to bootloader:
```
fastboot reboot bootloader
```
8. Now reboot to lineage recovery:
```
