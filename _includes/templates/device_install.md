...
{% include alerts/note.html content="Note: addon.d-v2, the system responsible for backing up addons such as GApps/AddonSU does NOT run in recovery on A/B (seamless update) devices. On A/B devices, addons are backed up if you update via the builtin LineageOS Updater app. There is a script in [this repo](https://github.com/LineageOS/android_packages_apps_Updater)" should you want to update from your PC. To further reiterate, if you sideload an update from recovery, your addons WILL NOT persist."%}
{% else %}
{% endif %}
...
{% if device.is_ab_device %}
2. If you aren't already booted into recovery, follow the steps above to do so.
{% else %}
2. If you aren't already booted into recovery, reboot into recovery:
