...
{% include alerts/important.html content="Please read through the entire instructions at least once before actually following them, so as to avoid any problems due of missed steps!" %}

1. Make sure your computer has new and working versions of `adb`{% unless device.install_method == 'heimdall' or device.install_method == 'dd' %} and `fastboot`{% endunless %}. Setup instructions can be found [here]({{ "adb_fastboot_guide.html" | relative_url }}).
...
{% endif %}
{% if device.is_ab_device %}
7. _(Optionally)_: If you want to install any other add-ons, run `adb reboot sideload`, then `adb sideload /path/to/zip` those packages in sequence.
{% else %}
7. _(Optionally)_: If you want to install any other add-ons, `adb sideload filename.zip` those packages in sequence.
{% endif %}
