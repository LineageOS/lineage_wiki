...
{% endif %}
    * Optionally, download additional application packages such as [Google Apps]({{ "gapps.html" | relative_url }}) (use the `{{ userspace_architecture }}` architecture), or the [LineageOS SU Addon](https://download.lineageos.org/extras).
...
{% if device.is_ab_device %}
10. Once installation has finished, return to the main menu, tap **Reboot**.
{% else %}
