...
Firmware is included in the stock ROM (the one the device comes with out of the box).
{%- endif %}
...
{%- capture content %}
{%- if device.before_install.version %}
If you are unsure what version you are currently on, we strongly recommend returning to the corresponding stock ROM of the required version, just in case!
