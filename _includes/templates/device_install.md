...
3. _(Optional, but recommended)_: Tap the **Backup** button to create a backup. Make sure to {% if device.sdcard != empty %}create the backup in the external sdcard or copy it{% else %}copy the backup{% endif %} into your computer, since the internal storage will be formatted.
