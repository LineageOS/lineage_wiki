...
```
    {% include alerts/note.html content="Newer fastboot releases dropped legacy A/B support thus it might attempt to flash `boot__a` / `boot__b` rather than `boot_a` / `boot_b`, in this case you will need to specify slot a when flashing images, e.g. `fastboot flash boot_a <recovery_filename>.img`" %}
