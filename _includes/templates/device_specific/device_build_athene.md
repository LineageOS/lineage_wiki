...
### Download proprietary blobs

Instead of extracting the proprietary blobs from device which are incomplete you can download directly from GitHub. Firstly go to `android/lineage/vendor/motorola`, erase the content of this folder, return to `android/lineage` and type the following command:

```
nano .repo/local_manifests/roomservice.xml
```
Add this line to the xml and resync the repo:
...
LC_ALL is the environment variable that overrides all the other localisation settings avoiding some compiling errors. Run this command:
