...
{% include alerts/warning.html content="Please note that both `aml-flash-tool` and `update` are known to be somewhat finicky on specific host platforms (e.g. AMD machines). If you encounter any failures while flashing, return to step 2 and try again." %}
 ...
 3. If your device reports `unlocked: no`, power off the device, ensure that USB is connected to your host machine, and boot it into burn mode by holding the pairing button on the side of the device while connecting to you host machine. If your device shows a "Fastboot" screen when long pressing the side-button, try again and continue holding it even after "Fastboot" appears on screen.
