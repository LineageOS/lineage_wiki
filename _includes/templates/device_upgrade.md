{%- assign device = site.data.devices[page.tdevice] %}
...
{% capture upgrade_only %}These instructions only apply to version upgrades. If ou wish to downgrade to an earlier version of LineageOS, follow your [device's]({{ path_prefix | append: "/install" | relative_url }}) instructions for installing LineageOS the first time.{% endcapture %}
