...
{%- else %}
1. Verify your device is using the latest [Lineage Recovery](https://download-beta.lineageos.org/devices/{{ custom_recovery_codename }}). Simply download the latest recovery file, named `recovery.img`.
{%- endif %}
...
1. Download the [LineageOS install package](https://download-beta.lineageos.org/devices/{{ device.codename }}) that you'd like to install or [build]({{ path_prefix | append: "/build" | relative_url }}) the package yourself.
{%- else %}
