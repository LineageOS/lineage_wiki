...
2. Make sure your computer has working `adb`{% unless device.install_method == 'heimdall' or device.install_method == 'dd' %} and `fastboot`{% endunless %}. Setup instructions can be found [here]({
...
6. _(Optionally)_: Run `adb reboot sideload` once more, then install any additional packages using the same method.
...
{% else %}
6. _(Optionally)_: Install any additional packages using the same method.
    {% include alerts/note.html content="If you previously had any Google Apps package installed on your device, you must install an updated package **before** the first boot of Android! If you did not have Google Apps installed, you must wipe the **Data** partition (or perform a factory reset) to install them." %}
{% endif %}
7. _(Optionally)_: Root the device by installing [LineageOS's AddonSU](https://download.lineageos.org/extras), use the {{ device.architecture.userspace }} version.
8. Once all installations are finished, run 'adb reboot'.
