...
5. Flash the super_empty partition to retrofit dynamic partitions to your device (inserting the path to your `super_empty.img` file):
    ...
    ```
    {% include alerts/note.html content="This process only needs to be performed once if you are upgrading from older LineageOS versions that still utilize the legacy partition layout." %}
    {% capture content -%}
