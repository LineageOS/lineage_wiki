...
{% include note.html content="Older build guides use the path `~/android/system/` instead of `~/android/lineage`. Remember to replace it with the one you actually used in any further references of this wiki." %}
...
`~/android/lineage/device/{{site.data.devices[page.device].vendor_short}}/{{site.data.devices[page.device].codename}}` folder. Then run the `extract-files.sh` script:

```
$ ./extract-files.sh
```

The blobs should be pulled into the `~/android/lineage/vendor/{{site.data.devices[page.device].vendor_short}}` folder. If you see "command not found" errors, `adb` may
