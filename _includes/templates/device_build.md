...
`~/android/lineage/device/{{ device.vendor_short }}/{{ device.codename }}` folder.

If you are building LineageOS 22 or newer, run `extract-files.py`:
```
./extract-files.py
```

If you are building LineageOS 21 or older, run `extract-files.sh`.
```
./extract-files.sh
```
