...
If you want the quickest way to building, then install run `apt-get install repo`.  If you plan on submitting changes to Gerrit, or your Ubuntu release does not have `repo`, then skip to installing `repo` via `curl`.

### Use `curl` to download latest `repo`
