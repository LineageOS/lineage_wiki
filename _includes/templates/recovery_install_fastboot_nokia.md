...
{% if device.custom_recovery_codename %}
## Temporary booting a custom recovery using `fastboot`
{% else %}
...
```
    ...
    You can also boot into download mode via a key combination:

    * {{ device.download_boot }}
    {% endif %}
4. Once the device is in download mode, verify your PC finds it by typing:
```
fastboot devices
```
    {% include alerts/tip.html content="If you see `no permissions fastboot` while on Linux or macOS, try running `fastboot` as root." %}
    {% if device.custom_recovery_link %}
5. Use fastboot to temporary boot directly into the custom recovery:
```
