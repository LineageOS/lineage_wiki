...
`aml-flash-tool` is Radxa's wrapper script for Amlogic's official `update` tool, which allows interfacing with burn mode.
`aml-flash-tool` is only available for Linux, though a VM will suffice.
